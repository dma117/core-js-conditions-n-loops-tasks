...
function convertToRomanNumerals(num) {
  let result = '';
  const operations = [(a) => a - (a % 10), (a) => a % 10];
  for (let i = 0; i < operations.length; i += 1) {
    const digit = operations[i](num);
    switch (digit) {
      case 1:
        result += 'I';
        break;
...
function rotateMatrix(matrix) {
  ...
  let k = -1;
  for (let j = 0; j < matrixCopy[0].length; j += 1) {
    k += 1;
    let l = 0;
    for (let i = matrixCopy.length - 1; i >= 0; i -= 1) {
      rotated[k][l] = matrixCopy[i][j];
      l += 1;
    }
  }
  return rotated;
...
function sortByAsc(arr) {
  ...
  let rightArr = new Array(arr.length - middle);
  for (let i = 0; i < arr.length; i += 1) {
    if (i < middle) {
      leftArr[i] = arr[i];
    } else {
...
function shuffleChar(str, iterations) {
  let result = str;
  let endCount = iterations;
  let iterationsCount = 0;
  for (let i = 0; i < endCount; i += 1) {
    let evenIndSubstr = '';
    let oddIndSubstr = '';
    for (let j = 0; j < result.length; j += 1) {
      if (j % 2 === 0) {
        evenIndSubstr += result[j];
      } else {
        oddIndSubstr += result[j];
      }
    }
    result = evenIndSubstr + oddIndSubstr;
    iterationsCount += 1;
    if (result === str) {
      i = -1;
      endCount = iterations % iterationsCount;
      iterationsCount = 0;
    }
  }
  return result;
  ...
  for (let i = digits.length - 2; i > 0; i -= 1) {
    if (digits[i] < digits[i + 1]) {
      ...
      for (let j = i + 2; j < digits.length; j += 1) {
        if (digits[j] > digits[i] && digits[j] < digits[minDigitInd]) {
          minDigitInd = j;
        }
      }
      const tmp = digits[i];
      digits[i] = digits[minDigitInd];
      digits[minDigitInd] = tmp;
      result = 0;
      const digitsCopy = [...digits];
      const left = digits.splice(0, i + 1);
      const sortedSubArray = sortByAsc(digits);
      for (let j = 0; j < i + 1; j += 1) {
        result += left[j] * 10 ** (digitsCopy.length - j - 1);
      }
      for (let j = 0; j < sortedSubArray.length; j += 1) {
        result += sortedSubArray[j] * 10 ** (sortedSubArray.length - j - 1);
      }
      break;
    }
  }
